...
function spriteUrl(sprite: SpriteAsset, assetEndpoint: string) {
  const bundledUrl = `/${sprite.path.replace(/^public\//, "")}`;
  return sprite.builtIn || !assetEndpoint ? bundledUrl : `${assetEndpoint}?path=${encodeURIComponent(sprite.path)}`;
}
...
export function WorldScene({ project, levelId, assetEndpoint = "/api/game/assets", hideObjectIds = [], selectedObjectId, showCollision = false, showDoors = true, onObjectPointerDown, onObjectContextMenu, onDoorClick, onDoorContextMenu }: {
  ...
  const worldStyle = { "--world-aspect": String(level.width / level.height) } as CSSProperties;
  return <div className={styles.world} style={worldStyle} aria-label={`${level.name} authored world`}><div className={styles.plane}>
