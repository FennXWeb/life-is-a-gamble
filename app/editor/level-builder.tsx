...
export function LevelBuilder({ project, edit, setNotice }: { project: GameProject; edit: Edit; setNotice: (notice: { tone: "ok" | "warn" | "bad"; text: string } | null) => void }) {
  ...
  return <div className={styles.builder}>
    ...
    <aside className={styles.builderInspector}>
      ...
      {selectedObject ? <ObjectInspector object={selectedObject} layers={layers} sprites={project.spriteAssets} update={(patch) => updateObject(selectedObject.id, patch)} duplicate={() => { const copy = { ...structuredClone(selectedObject), id: makeId(selectedObject.name, "object"), name: `${selectedObject.name} copy`, x: selectedObject.x + level.gridSize, y: selectedObject.y + level.gridSize }; edit((draft) => draft.levelObjects.push(copy)); setSelectedObjectId(copy.id); }} remove={() => { edit((draft) => { draft.levelObjects = draft.levelObjects.filter((entry) => entry.id !== selectedObject.id); }); setSelectedObjectId(""); }} /> : selectedCell ? <CellBuilderInspector cell={selectedCell} project={project} update={(patch) => updateCell(selectedCell.id, patch)} edit={edit} /> : activeLayer ? <LayerInspector layer={activeLayer} update={(patch) => updateLayer(activeLayer.id, patch)} remove={() => { if (layers.length <= 1) return; edit((draft) => { draft.levelObjects = draft.levelObjects.filter((entry) => entry.layerId !== activeLayer.id); draft.levelLayers = draft.levelLayers.filter((entry) => entry.id !== activeLayer.id); }); setLayerId(""); }} /> : <LevelInspector level={level} update={updateLevel} />}
    </aside>
  </div>;
}
